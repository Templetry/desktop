...
func (a *App) PublishProject(dir, name, description string, private bool) (string, error) {
	...
	if err := gitPublish(dir, repo.CloneURL, token, login); err != nil {
...
// The token travels via environment to a one-shot credential helper — never
// on the command line, never persisted in .git/config.
func gitPublish(dir, remoteURL, token, login string) error {
	email := login + "@users.noreply.github.com"
	run := func(args ...string) error {
		cmd := exec.Command("git", args...)
		cmd.Dir = dir
		cmd.Env = append(os.Environ(),
			"TEMPLETRY_GH_TOKEN="+token,
	...
	if err := run("-c", "user.name="+login, "-c", "user.email="+email, "add", "-A"); err != nil {
		return err
	}
	if err := run("-c", "user.name="+login, "-c", "user.email="+email,
		"commit", "-m", "Initial commit — generated by Templetry"); err != nil && !strings.Contains(err.Error(), "nothing to commit") {
		return err
	}
	if err := run("branch", "-M", "main"); err != nil {
		return err
	}
	_ = run("remote", "add", "origin", remoteURL)
	helper := `!f() { echo "username=oauth2"; echo "password=$TEMPLETRY_GH_TOKEN"; }; f`
	return run("-c", "credential.helper=", "-c", "credential.helper="+helper, "push", "-u", "origin", "main")
}
