...
func gitPublish(dir, remoteURL, token, login string) error {
	if _, err := os.Stat(filepath.Join(dir, ".git")); os.IsNotExist(err) {
		if err := runGit(dir, token, login, "init"); err != nil {
			return err
		}
	}
	if err := runGit(dir, token, login, "add", "-A"); err != nil {
		return err
	}
	if err := runGit(dir, token, login, "commit", "-m", "Initial commit — generated by Templetry"); err != nil && !strings.Contains(err.Error(), "nothing to commit") {
		return err
	}
	if err := runGit(dir, token, login, "branch", "-M", "main"); err != nil {
		return err
	}
	_ = runGit(dir, token, login, "remote", "add", "origin", remoteURL)
	return runGit(dir, token, login, "push", "-u", "origin", "main")
}
